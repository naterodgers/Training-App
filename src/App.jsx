...
const PLAN_TABS = [
  ...
  {id:"calendar", label:"Calendar",  icon:"🗓️", color:P.blue},
  {id:"volume",   label:"Volume",    icon:"📊", color:P.clay},
...
// VOLUME CALCULATOR
// ─────────────────────────────────────────────
function parseVolume(trainingText, raceCat, totalWks) {
  if (!trainingText) return [];
  const lines = trainingText.split("\n");
  const weeks = [];
  let curWeek = null;
  const parseDistance = (str) => {
    const m = str.match(/(\d+(?:\.\d+)?)\s*(mi|mile|km|k|yd|yard)\b/i);
    if (!m) return null;
    let val = parseFloat(m[1]);
    const unit = m[2].toLowerCase();
    if (unit === "km" || unit === "k") val *= 0.621;
    if (unit === "yd" || unit === "yard") val /= 1760;
    return val;
  };
  const classifyWorkout = (line) => {
    const l = line.toLowerCase();
    if (l.includes("swim") || l.includes("pool") || l.includes("open water")) return "swim";
    if (l.includes("bike") || l.includes("cycle") || l.includes("cycling") || l.includes("ride") || l.includes("trainer")) return "bike";
    return "run";
  };
  for (const line of lines) {
    const wkMatch = line.match(/\*\*Week\s*(\d+)/i);
    if (wkMatch) { curWeek = { week: parseInt(wkMatch[1]), run: 0, bike: 0, swim: 0, workouts: 0 }; weeks.push(curWeek); continue; }
    if (curWeek && (line.startsWith("\u2022") || line.startsWith("-") || line.match(/^\*\*(Mon|Tue|Wed|Thu|Fri|Sat|Sun)/i))) {
      const dist = parseDistance(line);
      if (dist) { const type = classifyWorkout(line); curWeek[type] += dist; curWeek.workouts++; }
    }
  }
  const filled = weeks.filter(w => w.run + w.bike + w.swim > 0);
  if (filled.length === 0 && totalWks) {
    const base = raceCat === "tri" ? { run: 8, bike: 30, swim: 1.5 } : raceCat === "ultra" ? { run: 20, bike: 0, swim: 0 } : { run: 15, bike: 0, swim: 0 };
    return Array.from({ length: Math.min(totalWks, 20) }, (_, i) => {
      const pct = i < totalWks * 0.6 ? 0.6 + (i / (totalWks * 0.6)) * 0.4 : i < totalWks * 0.85 ? 1 : 0.6;
      return { week: i + 1, run: +(base.run * pct).toFixed(1), bike: +(base.bike * pct).toFixed(1), swim: +(base.swim * pct).toFixed(1), workouts: 3, estimated: true };
    });
  }
  return weeks;
}
function aggregateByMonth(weeklyData, startDate) {
  if (!startDate || !weeklyData.length) return [];
  const months = {};
  weeklyData.forEach(w => {
    const d = new Date(startDate); d.setDate(d.getDate() + (w.week - 1) * 7);
    const key = d.toLocaleDateString("en-US", { month: "short", year: "numeric" });
    if (!months[key]) months[key] = { label: key, run: 0, bike: 0, swim: 0, weeks: 0 };
    months[key].run += w.run; months[key].bike += w.bike; months[key].swim += w.swim; months[key].weeks++;
  });
  return Object.values(months);
}
function parseCalendarEvents(trainingText, startDate) {
  if (!trainingText || !startDate) return {};
  const events = {};
  const lines = trainingText.split("\n");
  let curWeekNum = 0;
  ...
  for (const line of lines) {
    const wkMatch = line.match(/\*\*Week\s*(\d+)/i);
    if (wkMatch) { curWeekNum = parseInt(wkMatch[1]) - 1; continue; }
    const dayMatch = line.match(/^\*\*(Mon|Tue|Wed|Thu|Fri|Sat|Sun)/i);
    if (dayMatch && curWeekNum >= 0) {
      const dayOffset = dayMap[dayMatch[1].toLowerCase()];
      if (dayOffset === undefined) continue;
      const d = new Date(startDate); d.setDate(d.getDate() + curWeekNum * 7 + dayOffset);
      const key = d.toISOString().split("T")[0];
      const text = line.replace(/\*\*/g, "").replace(/^(Mon|Tue|Wed|Thu|Fri|Sat|Sun)[^:]*:\s*/i, "").trim();
      const isRest = /rest|recover|off/i.test(text);
      const type = /swim/i.test(text) ? "swim" : /bike|ride|cycl/i.test(text) ? "bike" : isRest ? "rest" : "run";
      events[key] = { text: text.slice(0, 60) + (text.length > 60 ? "\u2026" : ""), type, date: key };
    }
  }
  return events;
}
function generateICS(trainingText, startDate, raceName) {
  const events = parseCalendarEvents(trainingText, startDate);
  const icsDate = (s) => s.replace(/-/g, "");
  const esc = (s) => (s||"").replace(/,/g,"\\,").replace(/;/g,"\\;");
  const emoji = { run:"🏃", bike:"🚴", swim:"🏊", rest:"😴" };
  let ics = ["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//Day Won//EN","CALSCALE:GREGORIAN","METHOD:PUBLISH",`X-WR-CALNAME:${esc(raceName)} Training`];
  Object.values(events).forEach((ev,i) => {
    ics = ics.concat(["BEGIN:VEVENT",`UID:dw-${i}-${ev.date}@daywon.app`,`DTSTART;VALUE=DATE:${icsDate(ev.date)}`,`DTEND;VALUE=DATE:${icsDate(ev.date)}`,`SUMMARY:${emoji[ev.type]||"🏃"} ${esc(ev.text)}`,`DESCRIPTION:Day Won — ${esc(raceName)}`,"END:VEVENT"]);
  });
  ics.push("END:VCALENDAR");
  return ics.join("\r\n");
}
function downloadICS(trainingText, startDate, raceName) {
  const blob = new Blob([generateICS(trainingText, startDate, raceName)], { type:"text/calendar;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url; a.download = `${(raceName||"training").replace(/\s+/g,"-").toLowerCase()}-plan.ics`;
  document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
}

async function callAI(sys, user, tokens=4000) {
  const r=await fetch("https://api.anthropic.com/v1/messages",{
    method:"POST",headers:{"Content-Type":"application/json","x-api-key":import.meta.env.VITE_ANTHROPIC_KEY,"anthropic-dangerous-direct-browser-access":"true","anthropic-version":"2023-06-01"},
...
export default function App() {
  ...
  const [volView,   setVolView]  = useState("week");
    ...
    <div style={S.root}>
      ...
      <div style={S.planNav}>
        ...
        <div style={S.planNavR}>
          ...
          <button className="ghb" style={{...S.ghostBtn,color:icsExported?P.green:P.textMid}} onClick={()=>{const rn=sr?sr.name:raceType?.name;downloadICS(secs.training,params.startDate,rn);setIcsExported(true);setTimeout(()=>setIcsExported(false),3000);}}>
            {icsExported?"✓ Exported":"📆 Export Cal"}
          </button>
      ...
      <div style={S.planMain}>
        ...
        {tab==="calendar"&&(()=>{
          const calEvents=parseCalendarEvents(secs.training,params.startDate);
          const startD=params.startDate?new Date(params.startDate):new Date();
          const viewStart=new Date(startD); viewStart.setDate(1+calMonth*28);
          const monthLabel=viewStart.toLocaleDateString("en-US",{month:"long",year:"numeric"});
          const typeColor={run:P.green,bike:P.blue,swim:P.purple,rest:P.border2};
          const typeBg={run:P.greenDim,bike:P.blueDim,swim:P.purpleDim,rest:P.bg3};
          // Build 5-week grid from viewStart
          const gridStart=new Date(viewStart);
          gridStart.setDate(gridStart.getDate()-gridStart.getDay());
          const days=Array.from({length:35},(_,i)=>{const d=new Date(gridStart);d.setDate(d.getDate()+i);return d;});
          const raceLabel=params.raceDate;
          return (
            <div style={{padding:"16px 20px 48px"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                <button style={{...S.ghostBtn,padding:"5px 12px"}} onClick={()=>setCalMonth(m=>m-1)}>← Prev</button>
                <div style={{fontSize:14,fontWeight:800,color:P.text,fontFamily:"'Montserrat',sans-serif"}}>{monthLabel}</div>
                <button style={{...S.ghostBtn,padding:"5px 12px"}} onClick={()=>setCalMonth(m=>m+1)}>Next →</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:4}}>
                {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d=><div key={d} style={{textAlign:"center",fontSize:9,fontWeight:700,color:P.muted,fontFamily:"'Montserrat',sans-serif",padding:"4px 0",letterSpacing:"0.1em"}}>{d}</div>)}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
                {days.map((d,i)=>{
                  ...
                  const isRace=key===raceLabel;
                  const inRange=params.startDate&&params.raceDate&&key>=params.startDate&&key<=params.raceDate;
                  return (
                    <div key={i} style={{minHeight:64,background:isRace?P.greenDim:ev?typeBg[ev.type]:inRange?P.bg2:P.bg1,border:`1px solid ${isRace?"rgba(74,222,128,0.5)":isToday?"rgba(96,165,250,0.5)":P.border}`,borderRadius:6,padding:"4px 5px",position:"relative"}}>
                      <div style={{fontSize:9,fontWeight:700,color:isToday?P.blue:isRace?P.green:P.muted,marginBottom:3}}>{d.getDate()}</div>
                      {isRace&&<div style={{fontSize:8,fontWeight:800,color:P.green,fontFamily:"'Montserrat',sans-serif",lineHeight:1.2}}>🏁 RACE DAY</div>}
                      {ev&&!isRace&&<div style={{fontSize:9,color:typeColor[ev.type],lineHeight:1.3,wordBreak:"break-word"}}>{ev.text}</div>}
                    </div>
                  );
                })}
              </div>
              <div style={{display:"flex",gap:10,marginTop:12,flexWrap:"wrap"}}>
                {[["run",P.green,"🏃 Run"],["bike",P.blue,"🚴 Bike"],["swim",P.purple,"🏊 Swim"],["rest",P.muted,"😴 Rest"]].map(([t,c,l])=>(
                  <div key={t} style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:P.muted}}>
                    <div style={{width:10,height:10,borderRadius:2,background:c,opacity:0.7}}/>
                    {l}
                  </div>
                ))}
              </div>
              {!secs.training&&<div style={{...S.planEmpty,paddingTop:32}}><div style={{fontSize:32,marginBottom:10}}>🗓️</div><div style={{color:P.muted,fontSize:12,fontFamily:"'Montserrat',sans-serif"}}>Generate your plan to see calendar view</div></div>}
            </div>
          );
        })()}
        {tab==="volume"&&(()=>{
          const volData=parseVolume(secs.training,raceType?.cat,wks);
          const monthData=aggregateByMonth(volData,params.startDate);
          const viewData=volView==="week"?volData.slice(0,20):monthData;
          const isTri=raceType?.cat==="tri";
          const maxRun=Math.max(...viewData.map(w=>w.run||0),1);
          const maxBike=Math.max(...viewData.map(w=>w.bike||0),1);
          const maxSwim=Math.max(...viewData.map(w=>w.swim||0),1);
          const totalRun=volData.reduce((a,w)=>a+w.run,0);
          const totalBike=volData.reduce((a,w)=>a+w.bike,0);
          const totalSwim=volData.reduce((a,w)=>a+w.swim,0);
          const peakRun=Math.max(...volData.map(w=>w.run||0));
          const peakBike=Math.max(...volData.map(w=>w.bike||0));
          return (
            <div style={{padding:"16px 20px 48px"}}>
              {volData.length===0?<div style={S.planEmpty}><div style={{fontSize:32,marginBottom:10}}>📊</div><div style={{color:P.muted,fontSize:12,fontFamily:"'Montserrat',sans-serif"}}>Generate your plan to see volume data</div></div>:(
                <>
                  <div style={{display:"grid",gridTemplateColumns:isTri?"repeat(3,1fr)":"repeat(2,1fr)",gap:8,marginBottom:16}}>
                    <div style={{background:P.greenDim,border:"1px solid rgba(74,222,128,0.25)",borderRadius:10,padding:"12px 14px"}}>
                      <div style={{fontSize:18,fontWeight:800,color:P.green,fontFamily:"'Montserrat',sans-serif"}}>{totalRun.toFixed(0)} mi</div>
                      <div style={{fontSize:9,color:P.muted,textTransform:"uppercase",letterSpacing:"0.1em",fontFamily:"'Montserrat',sans-serif",marginTop:3}}>Total Run</div>
                      <div style={{fontSize:10,color:P.textMid,marginTop:4}}>Peak {peakRun.toFixed(0)} mi/wk</div>
                    </div>
                    {isTri&&<div style={{background:P.blueDim,border:"1px solid rgba(96,165,250,0.25)",borderRadius:10,padding:"12px 14px"}}>
                      <div style={{fontSize:18,fontWeight:800,color:P.blue,fontFamily:"'Montserrat',sans-serif"}}>{totalBike.toFixed(0)} mi</div>
                      <div style={{fontSize:9,color:P.muted,textTransform:"uppercase",letterSpacing:"0.1em",fontFamily:"'Montserrat',sans-serif",marginTop:3}}>Total Bike</div>
                      <div style={{fontSize:10,color:P.textMid,marginTop:4}}>Peak {peakBike.toFixed(0)} mi/wk</div>
                    </div>}
                    {isTri&&<div style={{background:P.purpleDim,border:"1px solid rgba(167,139,250,0.25)",borderRadius:10,padding:"12px 14px"}}>
                      <div style={{fontSize:18,fontWeight:800,color:P.purple,fontFamily:"'Montserrat',sans-serif"}}>{(totalSwim*1760).toFixed(0)} yd</div>
                      <div style={{fontSize:9,color:P.muted,textTransform:"uppercase",letterSpacing:"0.1em",fontFamily:"'Montserrat',sans-serif",marginTop:3}}>Total Swim</div>
                      <div style={{fontSize:10,color:P.textMid,marginTop:4}}>Across {wks||volData.length} weeks</div>
                    </div>}
                    {!isTri&&<div style={{background:P.bg2,border:`1px solid ${P.border}`,borderRadius:10,padding:"12px 14px"}}>
                      <div style={{fontSize:18,fontWeight:800,color:P.clay,fontFamily:"'Montserrat',sans-serif"}}>{volData.length}</div>
                      <div style={{fontSize:9,color:P.muted,textTransform:"uppercase",letterSpacing:"0.1em",fontFamily:"'Montserrat',sans-serif",marginTop:3}}>Weeks</div>
                      <div style={{fontSize:10,color:P.textMid,marginTop:4}}>Avg {(totalRun/Math.max(volData.length,1)).toFixed(0)} mi/wk</div>
                    </div>}
                  </div>
                  <div style={{display:"flex",gap:6,marginBottom:14}}>
                    {["week","month"].map(v=><button key={v} className="fp" style={{...S.fpill,padding:"5px 14px",fontSize:11,...(volView===v?S.fpillOn:{})}} onClick={()=>setVolView(v)}>{v==="week"?"Weekly":"Monthly"}</button>)}
                  </div>
                  <div style={{fontSize:10,fontWeight:700,color:P.muted,textTransform:"uppercase",letterSpacing:"0.12em",fontFamily:"'Montserrat',sans-serif",marginBottom:8}}>
                    {isTri?"Run / Bike / Swim volume":"Run volume"} — {volView==="week"?"per week":"per month"}
                    {volData[0]?.estimated&&<span style={{color:P.clay,marginLeft:6}}>(estimated)</span>}
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:4}}>
                    {viewData.map((w,i)=>{
                      const label=volView==="week"?`Wk ${w.week}`:w.label;
                      return (
                        <div key={i} style={{display:"flex",alignItems:"center",gap:8}}>
                          <div style={{width:38,fontSize:9,color:P.muted,fontFamily:"'Montserrat',sans-serif",textAlign:"right",flexShrink:0}}>{label}</div>
                          <div style={{flex:1,display:"flex",flexDirection:"column",gap:2}}>
                            {w.run>0&&<div style={{height:12,background:P.greenDim,borderRadius:2,display:"flex",alignItems:"center",position:"relative",width:`${(w.run/maxRun)*100}%`,minWidth:30}}>
                              <div style={{position:"absolute",right:4,fontSize:8,color:P.green,fontWeight:700}}>{w.run.toFixed(0)}mi</div>
                            </div>}
                            {isTri&&w.bike>0&&<div style={{height:12,background:P.blueDim,borderRadius:2,display:"flex",alignItems:"center",position:"relative",width:`${(w.bike/maxBike)*100}%`,minWidth:30}}>
                              <div style={{position:"absolute",right:4,fontSize:8,color:P.blue,fontWeight:700}}>{w.bike.toFixed(0)}mi</div>
                            </div>}
                            {isTri&&w.swim>0&&<div style={{height:12,background:P.purpleDim,borderRadius:2,display:"flex",alignItems:"center",position:"relative",width:`${(w.swim/maxSwim)*100}%`,minWidth:30}}>
                              <div style={{position:"absolute",right:4,fontSize:8,color:P.purple,fontWeight:700}}>{(w.swim*1760).toFixed(0)}yd</div>
                            </div>}
                          </div>
                        </div>
                      );
